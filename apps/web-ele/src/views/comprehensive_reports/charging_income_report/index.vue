...
import { getChargingIncomeReportApi } from '#/api/income/report';
...
// API数据获取函数
async function getChargingIncomeReportData(params?: {
  endTime?: string;
  enterprise?: string;
  ...
  try {
    const response = await getChargingIncomeReportApi(params);
    return {
      items: response.items,
    };
  } catch (error) {
    console.error('获取充电收益报表数据失败:', error);
    // 如果API调用失败，返回空数据
    return {
      items: [],
    };
...
function calculateFooterData(
  data: ChargingIncomeReportData[],
): ChargingIncomeReportData {
  const footerData: any = {
    time: $t('system.chargingIncomeReport.footer.total'),
  };
  ...
  Object.keys(firstRow).forEach((key) => {
    if (key !== 'time') {
      ...
      const total = data.reduce((sum, row) => {
        const value = Number(row[key as keyof ChargingIncomeReportData]) || 0;
        return sum + value;
      }, 0);
      footerData[key] = Number(total.toFixed(2)); // 数据保留2位小数
    }
  });
    ...
    schema: [
      {
        ...
        label: $t('system.chargingIncomeReport.form.enterpriseSelect'),
        componentProps: {
          class: 'w-full sm:max-w-xs md:max-w-sm lg:max-w-md xl:max-w-sm',
          placeholder: $t('system.chargingIncomeReport.form.enterprisePlaceholder'),
      ...
      },
      {
        ...
        fieldName: 'time',
        label: isMonthly ? $t('system.chargingIncomeReport.form.dateRange') : $t('system.chargingIncomeReport.form.monthRange'),
        componentProps: {
          class: 'w-full',
          startPlaceholder: isMonthly ? $t('system.chargingIncomeReport.form.startDate') : $t('system.chargingIncomeReport.form.startMonth'),
          endPlaceholder: isMonthly ? $t('system.chargingIncomeReport.form.endDate') : $t('system.chargingIncomeReport.form.endMonth'),
    ...
    proxyConfig: {
      ...
      ajax: {
        query: async (_params: any, formValues: any) => {
          ...
          const result = await getChargingIncomeReportData({
...
<template>
  <Page auto-content-height>
    <ElTabs v-model="activeTab" type="border-card" tab-position="left" class="charging-income-report-tabs">
      <ElTabPane :label="$t('system.chargingIncomeReport.tabs.monthly')" name="monthly">
        <MonthlyGrid
          :table-title="`${$t('system.chargingIncomeReport.title')} - ${$t('system.chargingIncomeReport.tabs.monthly')}`"
          :table-title-help="$t('system.chargingIncomeReport.description')"
        />
      </ElTabPane>
      <ElTabPane :label="$t('system.chargingIncomeReport.tabs.yearly')" name="yearly">
        <YearlyGrid
          :table-title="`${$t('system.chargingIncomeReport.title')} - ${$t('system.chargingIncomeReport.tabs.yearly')}`"
          :table-title-help="$t('system.chargingIncomeReport.description')"
        />
      </ElTabPane>
    </ElTabs>
