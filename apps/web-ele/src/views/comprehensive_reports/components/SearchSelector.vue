...
// 数据源函数类型
type DataSource = (keyword?: string) => Promise<Option[]> | Option[]
...
interface Emits {
  ...
  (e: 'change', value: string | number, option: Option): void
...
const currentOptions = computed(() => {
  return props.remote || props.dataSource ? internalOptions.value : props.options
})

const selectedOption = computed(() => {
  return currentOptions.value.find(option => option.value === props.modelValue)
})
...
const filteredOptions = computed(() => {
  ...
  return currentOptions.value.filter(option => {
    ...
    return option.label.toLowerCase().includes(searchKeyword.value.toLowerCase())
  })
})
...
const handleVisibleChange = (val: boolean) => {
  ...
  if (val) {
    ...
    nextTick(() => {
      // 聚焦搜索框
      const input = document.querySelector('.search-selector-content .el-input__inner') as HTMLInputElement
      input?.focus()
    })
  }
...
const scrollToHighlighted = () => {
  nextTick(() => {
    const highlightedElement = optionsListRef.value?.querySelector('.option-item-highlighted') as HTMLElement
...
const initializeData = async () => {
  if (props.dataSource && !props.remote) {
    ...
    // 使用静态选项
    if (props.modelValue === undefined && currentOptions.value.length > 0) {
...
watch(() => props.options, (newOptions) => {
  if (!props.dataSource && newOptions.length > 0) {
    const currentSelected = newOptions.find(option => option.value === props.modelValue)
    // 如果当前选中项不存在，且没有明确的 modelValue，选择第一项
    if (!currentSelected && props.modelValue === undefined) {
      const firstOption = newOptions[0]
      if (!firstOption.disabled) {
        console.log('Auto-selecting first enterprise (options changed):', firstOption)
        emit('update:modelValue', firstOption.value)
        emit('change', firstOption.value, firstOption)
      }
    }
...
/* 无数据状态 */
.no-data-state {
  @apply flex flex-col items-center justify-center gap-3 py-8 px-3;
  @apply text-muted-foreground;
}
