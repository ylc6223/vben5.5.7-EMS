...
import { getWindPowerIncomeReportApi } from '#/api/income/report';
...
// API数据获取函数
async function getWindPowerIncomeReportData(params?: {
  endTime?: string;
  enterprise?: string;
  ...
  try {
    const response = await getWindPowerIncomeReportApi(params);
    return {
      items: response.items,
    };
  } catch (error) {
    console.error('获取风电收益报表数据失败:', error);
    // 如果API调用失败，返回空数据
    return {
      items: [],
    };
...
function calculateFooterData(
  data: WindPowerIncomeReportData[],
): WindPowerIncomeReportData {
  const footerData: any = {
    time: $t('system.windPowerIncomeReport.footer.total'),
  };
  ...
  Object.keys(firstRow).forEach((key) => {
    if (key !== 'time') {
      ...
      const total = data.reduce((sum, row) => {
        const value = Number(row[key as keyof WindPowerIncomeReportData]) || 0;
        return sum + value;
      }, 0);
    ...
    schema: [
      {
        ...
        label: $t('system.windPowerIncomeReport.form.enterpriseSelect'),
        componentProps: {
          class: 'w-full sm:max-w-xs md:max-w-sm lg:max-w-md xl:max-w-sm',
          placeholder: $t('system.windPowerIncomeReport.form.enterprisePlaceholder'),
      ...
      },
      {
        ...
        fieldName: 'time',
        label: isMonthly ? $t('system.windPowerIncomeReport.form.dateRange') : $t('system.windPowerIncomeReport.form.monthRange'),
        componentProps: {
          class: 'w-full',
          startPlaceholder: isMonthly ? $t('system.windPowerIncomeReport.form.startDate') : $t('system.windPowerIncomeReport.form.startMonth'),
          endPlaceholder: isMonthly ? $t('system.windPowerIncomeReport.form.endDate') : $t('system.windPowerIncomeReport.form.endMonth'),
          rangeSeparator: $t('system.windPowerIncomeReport.form.separator'),
...
const createGridOptions = (reportType: ReportType) =>
  ...
  }) as VxeTableGridOptions<WindPowerIncomeReportData>;
    ...
    proxyConfig: {
      ...
      ajax: {
        query: async (_params: any, formValues: any) => {
          ...
          const result = await getWindPowerIncomeReportData({
    ...
    <ElTabs v-model="activeTab" type="border-card" tab-position="left" class="wind-power-income-report-tabs">
      <ElTabPane :label="$t('system.windPowerIncomeReport.tabs.monthly')" name="monthly">
        <MonthlyGrid
          ...
          :table-title-help="$t('system.windPowerIncomeReport.description')"
        />
      </ElTabPane>
      <ElTabPane :label="$t('system.windPowerIncomeReport.tabs.yearly')" name="yearly">
        <YearlyGrid
          :table-title="`${$t('system.windPowerIncomeReport.title')} - ${$t('system.windPowerIncomeReport.tabs.yearly')}`"
          :table-title-help="$t('system.windPowerIncomeReport.description')"
        />
      </ElTabPane>
    </ElTabs>
